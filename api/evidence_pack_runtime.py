...
DEFAULT_EVIDENCE_PACK_REGISTRY_PATH = (
    Path(__file__).resolve().parents[1]
    / "data"
    / "evidence_packs"
    / "registry"
    / "evidence_pack_registry.v1.json"
)
...
def _registry_modes() -> dict[str, dict[str, Any]]:
    return {k: v for k, v in (_evidence_pack_registry().get("modes") or {}).items() if isinstance(v, dict)}
...
def _registry_pack_versions() -> dict[str, dict[str, Any]]:
    return {
        str(version): config
        for version, config in (_evidence_pack_registry().get("pack_versions") or {}).items()
...
def _compat_promotion(mode: str) -> dict[str, Any]:
    ...
    metadata_equals = promotion.get("metadata_equals") or {}
    record_field_exact = promotion.get("record_field_exact") or {}
...
def _validate_manifest_promotion(mode: str, path: Path, raw_bytes: bytes | None, data: dict[str, Any], metadata: dict[str, Any]) -> tuple[str, ...]:
    ...
    for key, expected in (promotion.get("metadata_equals") or {}).items():
    ...
    for key, expected in (promotion.get("metadata_int_equals") or {}).items():
        try:
    ...
    for key, expected in (promotion.get("metadata_set_equals") or {}).items():
        if set(metadata.get(key) or []) != set(expected or []):
            warnings.append(_warning(prefix, f"{key}_mismatch"))
    for key, expected in (promotion.get("metadata_dict_equals") or {}).items():
        if dict(metadata.get(key) or {}) != dict(expected or {}):
            warnings.append(_warning(prefix, f"{key}_mismatch"))
    ...
    if expected_count is not None and len(records) != int(expected_count):
        warnings.append(_warning(prefix, "record_count_mismatch"))
    for field_name, expected_value in (promotion.get("record_field_exact") or {}).items():
    ...
    if unique_row_field:
        ...
        expected_rows = int(promotion.get("unique_row_count") or 0)
        if len(row_ids) != expected_rows:
            warnings.append(_warning(prefix, "row_count_mismatch"))
        expected_fields = sorted(str(field) for field in (promotion.get("exact_fields_per_row") or []))
        if expected_fields:
            ...
            for _row_id, fields in row_fields.items():
                if sorted(fields) != expected_fields:
                    ...
                    break
    expected_record_fields = set(str(field) for field in (promotion.get("record_fields_exact") or []))
    ...
    expected_verticals = set(str(vertical) for vertical in (promotion.get("record_verticals_exact") or []))
    ...
    allowed_statuses = set(str(status) for status in (promotion.get("source_golden_field_status_allowed") or []))
    if allowed_statuses:
