...
Stores jurisdiction-level metadata across tasks:
  - fee_formula (Task 1)
  - timezone (Task 6)
  - contact verification fields (Task 3)
  - gates and notes (Task 4)
...
def get_ahj_notes(city: str, state: str) -> list[str]:
    """Return note strings from AHJ record if present."""
    rec = get_ahj(city, state)
    if not rec:
        return []
    notes = rec.get("notes", [])
