...
def test_phase4b_healthcare_rules_do_not_leak_into_office_or_other_unpopulated_states():
    ...
    assert office["state_schema_context"]["coverage_level"] == "phase4c_ca_office_ti"
    assert office["state_schema_context"]["triggered_rules"]
    assert all(rule["id"].startswith("ca_office_") for rule in office["state_schema_context"]["triggered_rules"])
